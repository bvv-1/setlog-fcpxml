...
GUIでは素材フォルダを選んで `OK` を押すと、編集画面が開きます。クリップ一覧、代表フレーム、採用状態、in/out、メモを確認・保存できます。選択クリップの上下移動もできます。選択クリップのプレビュー画像は必要に応じて `.setlog/previews/` にPNGとして生成されます。回転メタデータ付きの動画は `.setlog/normalized/` に補正済み動画を作り、`連続プレビュー` とFCPXML書き出しではそちらを使います。歯車メニューから `.setlog` 配下のキャッシュを削除できます。
